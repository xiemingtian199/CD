...
class ProductionGenerator:
            ...
            rows.append(
                {
                    ...
                    "消费者选择的原因": self._ecommerce_choice_reason(theme, fields),
                    "产品解决了什么问题": self._consumer_problem(theme),
                    "为什么是这个产品来解决这个问题": self._ecommerce_why_this_product_reason(theme, fields),
                    "分析时间": timestamp,
                    "人工确认状态": "待确认",
                    "人工确认意见": "",
                    "确认人": "",
    ...
    def _ranked_selling_points(self, theme: dict[str, str], fields: dict[str, Any]) -> str:
        ...
        if self._is_desensitizing_toothpaste(fields):
            mapping = {
                "牙齿敏感护理": ["冷热酸甜敏感护理", "针对牙敏感日常困扰", "牙本质小管护理原理", "脱敏膏比普通牙膏更聚焦", "资质可查更安心"],
                "牙本质小管原理": ["脱敏护理看得懂", "针对敏感源头做护理联想", "成分有原理支撑", "买前更容易建立信任", "不靠夸张承诺"],
                "牙龈出血关注": ["牙龈状态日常关注", "刷牙出血人群更容易共鸣", "针对牙龈护理需求", "说明书范围内表达", "资质背书降低顾虑"],
                "牙菌斑管理": ["牙菌斑管理多一步", "饭后/睡前清洁补充", "日常口腔管理更细", "不只清新口气", "资质可查更放心"],
                "配方成分透明": ["成分让护理更有依据", "脱敏护理原理更好理解", "口腔管理方向更明确", "清新口感更好坚持", "成分表透明降低疑虑"],
                "居家日用步骤": ["使用步骤更清楚", "早晚居家容易坚持", "像日常刷牙一样顺手", "不用复杂操作", "按说明使用更安心"],
                "规格组合选择": ["试用/复购/囤货都能选", "先低门槛尝试", "家庭多人可备", "周期使用不断档", "组合选择更灵活"],
                "资质安心说明": ["买前先看资质", "医疗器械属性明确", "说明书信息可核验", "降低买错风险", "比普通口腔护理更有信任感"],
                "薄荷口感体验": ["薄荷清新口感", "入口体验更舒服", "日常使用更容易坚持", "护理后口腔感觉更清爽", "口感体验和功效分开表达"],
                "家庭备用复购": ["家庭备用更省心", "多人使用不容易断档", "周期护理更方便", "多支组合按需选", "复购心智更明确"],
            }
            points = mapping.get(theme["name"], [])
        elif "含漱" in product_text:
            mapping = {
                "日常口腔护理": ["日常护理多一步", "饭后/刷牙后都能用", "口腔状态更有掌控感", "使用动作简单", "资质可查更安心"],
                "正畸人群护理": ["正畸清洁后补充护理", "刷牙后多一步", "清洁流程更完整", "不增加复杂步骤", "使用方便"],
                "家庭备用护理": ["家庭常备更省心", "家人按需使用", "洗漱台随手可用", "日常小问题不用临时找", "购买选择清楚"],
                "饭后清爽护理": ["饭后口腔更清爽", "社交前状态更安心", "护理动作简单", "随手补充一步", "体验感明确"],
                "出行便携护理": ["出门也能随手护理", "外出保持口腔状态", "通勤/旅行场景适用", "不打断护理习惯", "收纳方便"],
                "成分信息透明": ["买前信息更透明", "成分和资质看得清", "适用范围清楚", "减少盲买疑虑", "不夸大功效"],
                "使用步骤清晰": ["使用步骤更简单", "首次购买容易理解", "按说明合理使用", "护理流程清楚", "降低使用门槛"],
                "规格组合清楚": ["按使用频率选组合", "先试用再囤货", "家庭备用更直观", "减少SKU疑虑", "价格选择清楚"],
                "资质安心说明": ["买前先看资质", "说明书/标签可展示", "医疗器械信息明确", "购买前降低疑虑", "页面更可信"],
    ...
    def _ecommerce_choice_reason(self, theme: dict[str, str], fields: dict[str, Any]) -> str:
        product_text = self._positioning_text(fields)
        if self._is_desensitizing_toothpaste(fields):
            mapping = {
                "牙齿敏感护理": "被冷热酸甜困扰，想找更针对牙敏感的护理方案",
                "牙本质小管原理": "不想盲买，看到原理和成分好处才觉得值得试",
                "牙龈出血关注": "刷牙出血让人焦虑，想找更稳妥、更有依据的护理选择",
                "牙菌斑管理": "不满足于只清新口气，想把日常口腔管理做得更细",
                "配方成分透明": "想知道成分能带来什么好处，降低盲买和踩雷感",
                "居家日用步骤": "希望护理不麻烦，能自然融入早晚刷牙习惯",
                "规格组合选择": "先试试是否适合，再决定复购或家庭囤货",
                "资质安心说明": "医疗器械类产品需要信任感，资质清楚才敢买",
                "薄荷口感体验": "希望入口舒服、用后清新，才更容易长期坚持",
                "家庭备用复购": "家里多人或周期护理，需要随手有、不断档",
            }
            return mapping.get(theme["name"], self._consumer_choice_reason(theme))
        if "含漱" in product_text:
            mapping = {
                "日常口腔护理": "饭后/刷牙后有即时需求，想让口腔状态更有掌控感",
                "正畸人群护理": "正畸清洁步骤多，想找简单的补充护理",
                "家庭备用护理": "家里常备更安心，临时需要时不用再找",
                "饭后清爽护理": "社交前、饭后在意口腔状态，清爽诉求直接",
                "出行便携护理": "外出也想保持护理习惯，不想因为场景变化中断",
                "成分信息透明": "买前想知道信息是否真实，减少医疗器械类产品疑虑",
                "使用步骤清晰": "首次购买怕麻烦，步骤越简单越容易转化",
                "规格组合清楚": "想先试用或按家庭用量买，减少买错的心理压力",
                "资质安心说明": "看到资质/说明书信息，购买风险感下降",
                "温和口感体验": "口感好坚持，适合做体验型购买理由",
            }
            return mapping.get(theme["name"], self._consumer_choice_reason(theme))
        return self._consumer_choice_reason(theme)

    def _ecommerce_why_this_product_reason(self, theme: dict[str, str], fields: dict[str, Any]) -> str:
        product_text = self._positioning_text(fields)
        if self._is_desensitizing_toothpaste(fields):
            mapping = {
                "牙齿敏感护理": "脱敏膏定位更聚焦牙敏感，成分与资质共同支撑信任",
                "牙本质小管原理": "生物活性玻璃等成分可解释为牙本质小管护理思路，让消费者理解为什么选它",
                "牙龈出血关注": "可宣传范围能承接牙龈状态关注，比普通清新口气产品更有针对性",
                "牙菌斑管理": "能从牙菌斑管理切入，不只停留在口气清新或普通清洁",
                "配方成分透明": "成分表完整，可把成分转化为脱敏护理、口腔管理、口感体验等具体好处",
                "居家日用步骤": "膏体形态接近日常刷牙，消费者不需要改变太多习惯",
                "规格组合选择": "可从试用到复购逐步选择，降低首次尝试风险",
                "资质安心说明": "医疗器械资质和说明书信息能降低消费者对功效和安全边界的担心",
                "薄荷口感体验": "薄荷相关成分能转化为清新口感，让护理更容易坚持",
                "家庭备用复购": "多支组合服务周期护理和家庭备用，解决不断档需求",
            }
            return mapping.get(theme["name"], self._why_this_product_reason(theme, fields))
        if "含漱" in product_text:
            mapping = {
                "日常口腔护理": "含漱液形态简单直接，能承接刷牙后/饭后的即时护理需求",
                "正畸人群护理": "用法轻，不增加复杂步骤，适合正畸后的日常补充",
                "家庭备用护理": "使用门槛低、家人都容易理解，适合家庭常备",
                "饭后清爽护理": "使用动作快，能承接饭后和社交前需求",
                "出行便携护理": "场景延展性强，可做通勤/旅行测试",
                "成分信息透明": "成分和资质信息能转化为“买得明白”的信任感",
                "使用步骤清晰": "步骤视觉化后，首次购买理解成本低",
                "规格组合清楚": "组合选择能覆盖试用、家庭备用和复购，降低买错压力",
                "资质安心说明": "资质图和说明书能增强医疗器械类产品信任",
                "温和口感体验": "入口体验是日常复购的重要理由",
            }
            return mapping.get(theme["name"], self._why_this_product_reason(theme, fields))
        return self._why_this_product_reason(theme, fields)
