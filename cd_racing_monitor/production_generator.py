...
class ProductionGenerator:
    ...
    def _ranked_selling_points(self, theme: dict[str, str], fields: dict[str, Any]) -> str:
        ...
        if self._is_desensitizing_toothpaste(fields):
            mapping = {
                "牙齿敏感护理": ["冷热酸甜敏感护理", "二类医疗器械/资质可查", "牙本质小管护理原理", "脱敏膏针对性更强", "120g/支规格清楚"],
                "牙本质小管原理": ["看懂脱敏护理原理", "牙本质小管护理逻辑", "生物活性玻璃", "氯化锶+氟化钠", "成分表清楚"],
                "牙龈出血关注": ["牙龈出血护理关注", "说明书适用范围内表达", "菌斑性牙龈炎症护理场景", "医疗器械资质背书", "按说明书使用"],
                "牙菌斑管理": ["牙菌斑管理多一步", "说明书范围内牙菌斑管理", "饭后/睡前护理补充", "口腔清洁管理更细", "资质信息可查"],
                "配方成分透明": ["成分信息清楚看", "生物活性玻璃", "氯化锶+氟化钠", "β-葡聚糖", "薄荷口感成分"],
                "居家日用步骤": ["使用步骤更清楚", "早晚居家都能用", "膏体形态接近日常习惯", "按说明书使用", "收纳方便"],
                "规格组合选择": ["1支/2支/3支/5支可选", "120g/支规格清楚", "试用/复购/囤货都能选", "家庭多人可备", "价格带覆盖更灵活"],
                "资质安心说明": ["资质信息清楚可查", "注册证/说明书可展示", "医疗器械属性明确", "页面信息更可信", "购买前降低疑虑"],
                "薄荷口感体验": ["薄荷清新口感", "薄荷油+薄荷脑", "留兰香油口感支撑", "日常使用更容易坚持", "口感体验和功效分开表达"],
                "家庭备用复购": ["家庭备用更省心", "多支组合按需选", "120g/支规格稳定", "周期护理不断档", "单支到多支选择灵活"],
            }
            points = mapping.get(theme["name"], [])
        elif "含漱" in product_text:
            mapping = {
                "日常口腔护理": ["日常护理多一步", "饭后/刷牙后都能用", "二类医疗器械/资质可查", "200ml规格清楚", "使用动作简单"],
                "正畸人群护理": ["正畸清洁后补充护理", "刷牙后多一步", "使用方便", "不增加复杂步骤", "规格清楚"],
    ...
    def _ecommerce_choice_reason(self, theme: dict[str, str], fields: dict[str, Any]) -> str:
        ...
        if self._is_desensitizing_toothpaste(fields):
            mapping = {
                "牙齿敏感护理": "痛点明确，看到牙敏感护理+资质背书就能快速判断",
                "牙本质小管原理": "想买得明白，成分和原理比单纯低价更能建立信任",
                "牙龈出血关注": "刷牙出血有焦虑，需要稳妥护理方向和资质支撑",
                "牙菌斑管理": "日常清洁后仍想加强管理，短卖点能直接对应需求",
                "配方成分透明": "成分党下单前会对比配方，信息透明降低犹豫",
                "居家日用步骤": "怕不会用，看到步骤清楚更容易下单",
                "规格组合选择": "按试用、复购、家庭备用选择，不用纠结买几支",
                "资质安心说明": "医疗器械类产品先看资质，背书越清楚越放心",
                "薄荷口感体验": "担心不好入口，清新口感能降低使用阻力",
                "家庭备用复购": "多人或周期使用，需要组合清楚、不断档",
            }
            return mapping.get(theme["name"], self._consumer_choice_reason(theme))
        if "含漱" in product_text:
            mapping = {
                "日常口腔护理": "饭后/刷牙后有即时场景，看到“多一步护理”容易被触发",
                "正畸人群护理": "正畸清洁麻烦，需要方便补充护理",
                "家庭备用护理": "家庭常备心智强，规格清楚就容易加入购物车",
                "饭后清爽护理": "社交前、饭后痛点强，清爽诉求直接",
                "出行便携护理": "外出也想保持护理习惯，需要随手可用",
                "成分信息透明": "买前看成分和资质，减少医疗器械类产品疑虑",
                "使用步骤清晰": "首次购买怕麻烦，步骤越简单越容易转化",
                "规格组合清楚": "想按预算和人数选组合，避免SKU理解成本",
    ...
    def _ecommerce_why_this_product_reason(self, theme: dict[str, str], fields: dict[str, Any]) -> str:
        ...
        if self._is_desensitizing_toothpaste(fields):
            mapping = {
                "牙齿敏感护理": "有脱敏膏品类定位、成分依据、资质背书，比普通牙膏更聚焦牙敏感",
                "牙本质小管原理": "生物活性玻璃+氯化锶+氟化钠能支撑原理型主图表达",
                "牙龈出血关注": "可宣传范围覆盖牙龈出血相关护理，能与普通清新口气产品区分",
                "牙菌斑管理": "可围绕牙菌斑管理做短标签，不只卖口感或清新",
                "配方成分透明": "成分表完整，适合做理性对比和成分背书",
                "居家日用步骤": "膏体形态接近日常刷牙习惯，使用门槛低",
                "规格组合选择": "1支到5支覆盖试用、复购、囤货，价格带选择灵活",
                "资质安心说明": "医疗器械资质和说明书信息可做强信任模块",
                "薄荷口感体验": "薄荷油、薄荷脑、留兰香油能支撑清新体验表达",
                "家庭备用复购": "120g/支+多支组合，适合家庭备用和周期复购",
            }
            return mapping.get(theme["name"], self._why_this_product_reason(theme, fields))
        if "含漱" in product_text:
            mapping = {
                "日常口腔护理": "含漱液形态简单直接，适合刷牙后/饭后补充护理",
                "正畸人群护理": "用法轻，不增加复杂步骤，适合正畸后的日常补充",
                "家庭备用护理": "200ml/瓶放在洗漱台好识别，适合家庭常备",
                "饭后清爽护理": "使用动作快，能承接饭后和社交前需求",
                "出行便携护理": "场景延展性强，可做通勤/旅行测试",
                "成分信息透明": "成分、规格、资质信息能形成信任组合",
                "使用步骤清晰": "步骤视觉化后，首次购买理解成本低",
                "规格组合清楚": "瓶数组合和价格带清楚，便于转化测试",
